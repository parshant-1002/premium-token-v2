...
import { CREATE_AIRDROP_REQUEST, GET_BLOCKCHAIN_DATA, GET_CONTENT_REQUEST, GET_LANGUAGE_OPTIONS, GET_WINNERS_LIST } from "../constants";
...
function* getLanguageOptions({ callback = () => { } }) {
    try {
        yield put(startLoading());
        const { data, status } = yield axios({
            method: API_METHODS.GET,
            url: `${API_URLS.GET_LANGUAGE_OPTIONS}`,
        });
        if (status !== STATUS_CODES.SUCCESS) {
            yield put(stopLoading());
            callback(data.msg, STATUS.ERROR);
...
function* ContentManagementSaga() {
    ...
    yield all([takeEvery(GET_LANGUAGE_OPTIONS, getLanguageOptions)]);
}
